...
def go_to_goal (xgoal, ygoal):
	...
	while(True):
		...
		p1=[x,y]
		q1=[xgoal,ygoal]
		if (math.sqrt(((x-x2)**2)+((y-y2)**2))<2.5):
			p2=[x2-0.5*math.cos(theta2),y2-0.5*math.sin(theta2)]
			if theta2<0:
				theta2=2*math.pi+theta2
			q2=[x2+2.5*math.cos(theta2),y2+2.5*math.sin(theta2)]
			if (doIntersect(p1,q1,p2,q2)):
				conflicto.append('2')
				p=True
		if math.sqrt(((x-x3)**2)+((y-y3)**2))<2.5:
			p2=[x3-0.5*math.cos(theta3),y3-0.5*math.sin(theta3)]
			if theta3<0:
				theta3=2*math.pi+theta3
			q2=[x3+2.5*math.cos(theta3),y3+2.5*math.sin(theta3)]
			if doIntersect(p1,q1,p2,q2):
				conflicto.append('3')
				p=True
		if math.sqrt(((x-x4)**2)+((y-y4)**2))<2.5:
			p2=[x4-0.5*math.cos(theta4),y4-0.5*math.sin(theta4)]
			if theta4<0:
				theta4=2*math.pi+theta4
			q2=[x4+2.5*math.cos(theta4),y4+2.5*math.sin(theta4)]
			if doIntersect(p1,q1,p2,q2):
				conflicto.append('4')
				p=True
		if math.sqrt(((x-x5)**2)+((y-y5)**2))<2.5:
			p2=[x5-0.5*math.cos(theta5),y5-0.5*math.sin(theta5)]
			if theta5<0:
				theta5=2*math.pi+theta5
			q2=[x5+2.5*math.cos(theta5),y5+2.5*math.sin(theta5)]
			if doIntersect(p1,q1,p2,q2):
				conflicto.append('5')
				p=True
		if math.sqrt(((x-x6)**2)+((y-y6)**2))<2.5:	
			p2=[x6-0.5*math.cos(theta6),y6-0.5*math.sin(theta6)]
			if theta6<0:
				theta6=2*math.pi+theta6
			q2=[x6+2.5*math.cos(theta6),y6+2.5*math.sin(theta6)]
			if doIntersect(p1,q1,p2,q2):
				conflicto.append('6')
				p=True
